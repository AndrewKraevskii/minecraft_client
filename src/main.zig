            ...
            inline else => |id| blk: {
                ...
                inline for (@typeInfo(T).@"struct".fields) |field| {
                    ...
                    switch (F) {
                        ...
                        String => {
                            if (skip) {
                                std.log.debug("skip string", .{});
                                try skipString(reader);
                            } else {
                                @field(result, field.name) = .fromUtf8(try readString(reader, arena));
                                std.log.debug("read string {s}", .{@field(result, field.name).utf8});
                            }
                        },
                        []const String => {
                        ...
                        []const u8 => {
                            ...
                            std.log.debug("skiped {d} bytes", .{len});
                        },
                        ?Slot => {
                        ...
                        []const ?Slot => {
                            ...
                            switch (@typeInfo(F)) {
                                .int => {
                                    ...
                                    std.log.debug("read int {s}: {d}", .{ field.name, @field(result, field.name) });
                                },
                                .@"enum" => {
                                ...
                                .float => |f| {
                                    ...
                                    std.log.debug("read float {d}", .{@field(result, field.name)});
                                },
                                .bool => {
                                    @field(result, field.name) = try reader.readInt(u8, .big) != 0;
                                    std.log.debug("read bool {}", .{@field(result, field.name)});
                                },
                                .@"struct" => |s| {
...
pub fn runClient(gpa: std.mem.Allocator, ip: []const u8, port: u16) !void {
    ...
    try handshake(arena.allocator(), stream, "Andrew", ip, port);
